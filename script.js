...
var keyVal=document.getElementById('_akg_key_val');
var copyBtn=document.getElementById('_akg_copy');
document.getElementById('_akg_close').onclick=function(){ov.remove();};
ov.onclick=function(e){if(e.target===ov)ov.remove();};
copyBtn.onclick=function(){navigator.clipboard&&navigator.clipboard.writeText(keyVal.textContent);copyBtn.textContent='\u2713 COPIED!';setTimeout(function(){copyBtn.textContent='\u2398 COPY KEY';},2000);};
function runBypass(){
  ...
  numEl.textContent=s;
  statusEl.textContent='\u23f3 Waiting to bypass...';
  var iv=setInterval(function(){
    ...
    if(s<=0){
      ...
      statusEl.textContent='\ud83d\ude80 Bypassing now...';
      var p=location.protocol+'//tarviral.com';
      fetch(p+'/api/session-info',{credentials:'include',headers:{'Accept':'*/*'}})
      .then(function(r){return r.json();})
      .then(function(d){
        if(!d.sessionToken){statusEl.textContent='\u274c No session found!';return;}
        var inp2=encodeURIComponent(JSON.stringify({"0":{"json":{"token":d.sessionToken,"progress":6,"stageId":d.stageId}}}));
        fetch(p+'/api/trpc/linkSession.nextStage?batch=1&input='+inp2,{credentials:'include',headers:{'trpc-accept':'application/jsonl','x-trpc-source':'nextjs-react','Accept':'*/*'}})
        .then(function(r){return r.text();})
        .then(function(t){
          var dest=null;
          t.trim().split('\n').forEach(function(l){try{var j=JSON.parse(l);if(j&&j.json&&Array.isArray(j.json)&&j.json[2]){var dd=j.json[2][0][0];if(dd&&dd.destinationLink)dest=dd.destinationLink;}}catch(e){}});
          if(dest){statusEl.textContent='\u2705 Done! Redirecting...';setTimeout(function(){ov.remove();window.location.href=dest;},1000);}
...
btn.onclick=function(){
  ...
  err.textContent='';
  if(h.includes('tarviral.com')){runBypass();}
  else if(h.includes('rodaemotor.com')){var links=document.querySelectorAll('a');var found=null;links.forEach(function(l){var txt=l.textContent.toUpperCase();if(txt.includes('AQUI')||txt.includes('CONTIN')){found=l;}});ov.remove();if(found){found.click();}else{window.location.href='https://alpharede.com/aincrad2';}}
  else if(h.includes('aincradmods.com')){ov.remove();fetch('https://aincradmods.com/getkey.data',{method:'POST',credentials:'include',headers:{'content-type':'application/x-www-form-urlencoded;charset=UTF-8','accept':'*/*','origin':'https://aincradmods.com','referer':'https://aincradmods.com/getkey','x-requested-with':'mark.via.gp'}}).then(function(){window.location.href='https://alpharede.com/aincrad2';}).catch(function(){window.location.href='https://alpharede.com/aincrad2';});}
  else if(h.includes('alpharede.com')){ov.remove();window.location.href='https://alpharede.com/aincrad2';}
  else{err.textContent='\u26a0 Open aincradmods.com/getkey first!';}
};
